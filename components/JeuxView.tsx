...
interface JeuxViewProps {
  onGenerateGame: (subject: Subject) => void;
}

export const JeuxView: React.FC<JeuxViewProps> = ({ onGenerateGame }) => {
  const { t } = useLocalization();
  return (
    <div className="w-full max-w-4xl mx-auto">
      <header className="text-center mb-12">
        ...
        <p className="text-xl text-slate-700 dark:text-slate-300 mt-2">
          Choisissez une matière pour générer un jeu !
        </p>
      </header>
      <main>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {SUBJECTS.filter(s => s.nameKey !== 'subject_games' && s.nameKey !== 'subject_arts').map((subject) => (
            <SubjectCard
              key={subject.nameKey}
              subject={subject}
              onSelect={() => onGenerateGame(subject)}
            />
          ))}
        </div>
      </main>
